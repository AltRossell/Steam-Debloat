...
public class UninstallService : IDisposable
{
        ...
        try
        {
            ...
            if (File.Exists(configPath))
            {
                ...
                if (content.Contains("BootStrapperInhibitAll=enable") || content.Contains("Mode:"))
                {
                    ...
                    {
                        DeleteFileWithPkexec(configPath);
                    }
                    else
                    {
    ...
    private void DeleteFileWithPkexec(string filePath)
    {
        try
        {
            ...
            {
                throw new Exception($"pkexec failed with exit code {process.ExitCode}");
            }
        }
        catch (Exception ex)
        {
            OnProgressChanged($"Warning: Could not delete file with pkexec: {ex.Message}");
            try
            {
            ...
            {
                throw new Exception($"Failed to delete config file. Please run the application with sudo or pkexec.");
            }
        }
    }
    ...
    private void CleanTempBatchFiles()
    {
        try
        {
            ...
            string[] tempFiles = { "Steam.bat", "Steam2025.bat", "Steam2022.bat", "Steam2023.bat", "Steam.sh" };
